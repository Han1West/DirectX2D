...
void CSprite::CopySprite(Ptr<CSprite> _Sprite)
{
	m_Atlas = _Sprite->GetAtlasTexture();
	m_LeftTopUV = _Sprite->GetLeftTop();
	m_SliceUV = _Sprite->GetSlice();
	m_BackgroundUV = _Sprite->GetBackground();
	m_OffsetUV = _Sprite->GetOffset();
	SetRelativePath(_Sprite->GetRelativePath());
}
