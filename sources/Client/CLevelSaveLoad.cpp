...
int CLevelSaveLoad::SaveLevel(const wstring& _FilePath, CLevel* _Level)
{
	...
	// 상대 경로를 이름으로 정한다.
	wstring RelativePath = _FilePath.substr(_FilePath.find(L"Level"));
	if(RelativePath != L"Level\\Temp.lv")
		_Level->SetRelativePath(RelativePath);

	// Level 이름 저장
	_Level->SaveToLevel(pFile);
	// Level 경로 저장
	_Level->SaveRelativePath(pFile);
...
CLevel* CLevelSaveLoad::LoadLevel(const wstring& _FilePath)
{
	...
	// Level 경로 불러오기
	pNewLevel->LoadRelativePath(pFile);
