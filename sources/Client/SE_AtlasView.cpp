...
void SE_AtlasView::Render_Update()
{
	...
	{
		GetDetail()->SetSpriteInfo(Vec2(m_MouseLT.x, m_MouseLT.y), Vec2(m_MouseRB.x, m_MouseRB.y));
	}
}
...
void SE_AtlasView::SelectCheck()
{
	...
	// 마우스가 텍스쳐 위에 있을 때만 클릭을 받는다.
	if (0.f <= vDiff.x && vDiff.x <= m_AtlasTex->GetWidth()
		&& 0.f <= vDiff.y && vDiff.y < m_AtlasTex->GetHeight())
	{
		// 마우스 왼쪽 Tap 체크
...
void SE_AtlasView::SetRectPos(Vec2 _LeftTop, Vec2 _Slice)
{
	m_MouseLT = ImVec2(_LeftTop.x, _LeftTop.y);
	m_MouseRB = ImVec2(_LeftTop.x + _Slice.x, _LeftTop.y + _Slice.y);
}